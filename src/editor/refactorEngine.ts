...
export const refactorSkillCode = (code: string): RefactorResult => {
  ...
  if (foreachRegex.test(refactored)) {
    refactored = refactored.replace(foreachRegex, (match, item, list) => {
      return `; Optimized with mapcar\nresult = mapcar('lambda((${item}) ...) ${list})`;
    });
    explanations.push("Optimized 'foreach' loops with 'cons' accumulation into 'mapcar' for better performance.");
  }
  ...
  if (trailingWhitespaceRegex.test(refactored)) {
    refactored = refactored.replace(trailingWhitespaceRegex, '');
    explanations.push("Removed trailing whitespace for cleaner code.");
  }
